...
- Сообщение `Worker.finish_job отсутствует в текущей версии pyjobkit; пропускаем патч` означает, что установленная версия библиотеки больше не содержит метода `finish_job`. Лоадтест в этом случае не вносит изменений в воркер и продолжает работу с новой реализацией `pyjobkit` без дополнительного патча.
- Исторически патч `_patch_worker_finish_version` писался под ранние версии `pyjobkit` (до перехода на публичный метод `finish`). Там `finish_job` был точкой входа в завершение задачи, и нам нужно было вручную синхронизировать версии job. В новых релизах библиотека сама решает вопрос версий, поэтому при отсутствии `finish_job` патч просто пропускается.
